...
Now that we have synced up our repositories, we should **sync up local and remote branches (if necessary)**. This will make our lives easier as we move forward.
	a. Type the command `git show-ref`. You should see something like this:

<img src="./assets/images/show-ref.jpg" width="200" align='center'/>

	b. Notice above that there are two refs - you can see the name of each branch as the last item in the ref path. Importantly in this example, **the refs are not the same**. <u>If your ref branches are the same, move on to step 6</u>

	c. If your refs are not the same like mine were in this instance, type the following two commands:
...
```
	...
	d. Now, if you run the command `git show-ref` again, you should have local and remote refs with the same branch name.
